...
def create_show(show_name, code_name, show_type):
    ...
    show_id_elements = [show_name, code_name, show_type]
    show_id_comp = str(".".join(show_id_elements))
...
def create_asset(show_name, category, entry_name, status ='NOT-STARTED', definition = xvalid.DEFAULT_ASSET_DEFINITION):
    db = xcon.server.xchange
    id_elements = [show_name, category, entry_name]
    id_comp = str('.'.join(id_elements))
    db.assets.insert(
        {
            "_id": id_comp,
            "show_name": show_name,
...
if __name__=="__main__":
    ...
    sel_elements = [show_name, category_name, entity_name]
    sel_id = str(".".join(sel_elements))
    print(sel_id)
    test_id = db.assets.find({"_id":sel_id},{"definition":1})
    for item in test_id:
        pprint.pprint(item)

    # create_show("DARK", "DRK", "vfx")
    # import pprint
    # import os
    # import getpass
    #
